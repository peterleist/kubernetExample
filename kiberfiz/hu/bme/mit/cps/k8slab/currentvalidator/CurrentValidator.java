...
package hu.bme.mit.cps.k8slab.currentvalidator;
...
import hu.bme.mit.cps.k8slab.CurrentDataWriter;
...
public class CurrentValidator {
    ...
    private static ArrayList<Current> data;
    
    private static ArrayList<Current> initData() {
    	Current instance0 = new Current();
		instance0.TimeStamp = 0;
		instance0.ISensorID = -1;
		instance0.IValue = 0;
		
		Current instance1 = new Current();
		instance1.TimeStamp = 0;
		instance1.ISensorID = -1;
		instance1.IValue = 0;
		
		Current instance2 = new Current();
		instance2.TimeStamp = 0;
		instance2.ISensorID = -1;
		instance2.IValue = 0;
		
		ArrayList<Current> list = new ArrayList<Current>(3);
		list.add(instance0);
		list.add(instance1);
		list.add(instance2);
    ...
    private static Current validate() {
		...
		if(n >= 2) {
			...
			System.out.println("Data0: " + data.get(0) + " Data1: " + data.get(1) + "Data2: " + data.get(2));
			if (data.get(0).IValue == data.get(1).IValue && data.get(1).IValue == data.get(2).IValue) {
				instance = new Current();
				instance.ISensorID = 1111;
				instance.IValue = data.get(0).IValue;
				instance.TimeStamp = Math.max(data.get(0).TimeStamp, Math.max(data.get(1).TimeStamp, data.get(2).TimeStamp));
				data = initData();
				
			}else if (data.get(0).IValue == data.get(1).IValue) {
				instance = new Current();
				instance.ISensorID = 1011;
				instance.IValue = data.get(0).IValue;
				instance.TimeStamp = Math.max(data.get(0).TimeStamp, data.get(1).TimeStamp);
				data = initData();
				
			}else if (data.get(0).IValue == data.get(2).IValue) {
				instance = new Current();
				instance.ISensorID = 1101;
				instance.IValue = data.get(0).IValue;
				instance.TimeStamp = Math.max(data.get(0).TimeStamp, data.get(2).TimeStamp);
				data = initData();
				
			}else if (data.get(1).IValue == data.get(2).IValue) {
				instance = new Current();
				instance.ISensorID = 1110;
				instance.IValue = data.get(1).IValue;
    ...
    private static void subscriberMain(int domainId, int sampleCount) {
        ...
        CurrentDataReader reader = null;
        CurrentDataWriter writer = null;
        ...
        try {
            ...
            String typeName = CurrentTypeSupport.get_type_name(); 
            CurrentTypeSupport.register_type(participant, typeName);
            ...
            ++count) {
                ...
                Current valid = validate();
                
                if(valid != null) {
                	System.out.println("Valid Current" + valid.toString());
        ...
        public void on_data_available(DataReader reader) {
            ...
            try {
                ...
                for(int i = 0; i < _dataSeq.size(); ++i) {
                    ...
                    if (info.valid_data) {
                        Current instance = new Current(_dataSeq.get(i));
                        
                        if(instance.ISensorID < 999) {
                        	data.set(instance.ISensorID, instance);
                        }
                    }
                }
