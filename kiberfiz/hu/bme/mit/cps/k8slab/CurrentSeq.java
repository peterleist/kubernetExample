...
/**
* A sequence of Current instances.
*/
public final class CurrentSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CurrentSeq (int initialMaximum) {
        super(Current.class, initialMaximum);
    }

    public CurrentSeq (Collection<?> elements) {
        super(Current.class, elements);
    }

    public  Current get(int index) { 
        return (Current) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Current.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Current objects (beyond the original size of this object)
