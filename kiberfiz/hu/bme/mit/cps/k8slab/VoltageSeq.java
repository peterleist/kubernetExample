...
/**
* A sequence of Voltage instances.
*/
public final class VoltageSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public VoltageSeq (int initialMaximum) {
        super(Voltage.class, initialMaximum);
    }

    public VoltageSeq (Collection<?> elements) {
        super(Voltage.class, elements);
    }

    public  Voltage get(int index) { 
        return (Voltage) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Voltage.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Voltage objects (beyond the original size of this object)
